...
import com.vicomo.currencyconverter.fakes.*
...
class CurrencyRepoImplTest {
    ...
    private lateinit var webService: FakeWebService
    // Class under test
    ...
    fun setUp(){
        ...
        webService = FakeWebService(remoteCurrencyResp, remoteExchangeRates, behaviourDelegate())
    ...
    fun loadExchangeRates_withNoCachedData_remoteDataIsCached() = runBlockingTest {
        ...
        currencyRepository.loadRemoteExchangeRates {
            runBlockingTest{
                ...
                MatcherAssert.assertThat(cachedData != null, CoreMatchers.`is`(true))
                MatcherAssert.assertThat(cachedData!!.quotes, CoreMatchers.`is`(remoteExchangeRates.quotes))
                MatcherAssert.assertThat(cachedData.source, CoreMatchers.`is`(remoteExchangeRates.source))
                MatcherAssert.assertThat(cachedData.timestamp, CoreMatchers.`is`(remoteExchangeRates.timestamp))
                MatcherAssert.assertThat(cachedData.quotes, CoreMatchers.`is`(remoteExchangeRates.quotes))
                MatcherAssert.assertThat(cachedData.quotes["USD"], CoreMatchers.`is`(remoteExchangeRates.quotes["USD"]))
                MatcherAssert.assertThat(cachedData.quotes["GBP"], CoreMatchers.`is`(remoteExchangeRates.quotes["GBP"]))
            }
        }
    ...
    fun loadCurrencies_forceRemote_retrievesRemoteData() = runBlockingTest {
        ...
        val currenciesDao = FakeCurrenciesDao(ArrayList(currentCurrencies)) //very important you clone currentCurrencies
        val webService = FakeWebService(currentCurrencyResp, remoteExchangeRates, behaviourDelegate())
        val currencyRepository = CurrencyRepoImpl(webService, currenciesDao, exchangeRateDao)
        currencyRepository.dispatcher = Dispatchers.Unconfined //very important
        //confirm
    ...
    fun loadExchangeRates_forceRemote_retrievesRemoteData() = runBlockingTest {
        ...
        val webService = FakeWebService(null, currentExchangeRates, behaviourDelegate())
    ...
    fun testRepoCleanSlate() = runBlockingTest {
        ...
        val webService = FakeWebService(null, null, behaviourDelegate())
        val currencyRepository = CurrencyRepoImpl(webService, FakeCurrenciesDao(arrayListOf()), FakeExchangeRateDao(null))
        currencyRepository.dispatcher = Dispatchers.Unconfined
    ...
    fun registerExchangeRatesCallback_refreshExchangeRates_callBackIsCalled() {
        ...
        val refreshCallback: (CurrencyExchangeRate?) -> Unit = {
            invokeCount++
            lock.countDown()
            lock = CountDownLatch(1) //vry important to reinitialize the lock for further refresh calls
        }
    ...
    * */
    private fun behaviourDelegate(): BehaviorDelegate<WebService> {
        ...
        val mockRetrofit = MockRetrofit.Builder(retrofit)
            ...
            .build()
        return mockRetrofit.create(WebService::class.java)
    }
}