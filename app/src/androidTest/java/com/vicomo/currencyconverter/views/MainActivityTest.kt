...
import com.vicomo.currencyconverter.fakes.currencyAmountList
...
class MainActivityTest {
    ...
    @get:Rule
    val activityRule = object: ActivityTestRule<MainActivity>(MainActivity::class.java,true, false){
    ...
    fun havingData_listDisplayed(){
        ...
        //GIVEN:
        viewModel.data.postValue(Data.success(currencyAmountList))
        //THEN
    ...
    fun onEmptyData_displayFeedback() {
        ...
        viewModel.data.postValue(Data.success(listOf()))
        //THEN:
//        onView(withText("Empty Data")).check(matches(isDisplayed()))
    ...
    fun compute_displayData(){
        ...
        viewModel.currencies.postValue(Data.success(cachedCurrencies))
        viewModel.exchangeRateCalculator = ExchangeRateCalculator(cachedExchangeRates, cachedCurrencies)
        //CONFIRM STATE :
        onView(withId(R.id.recyclerView)).check(matches(not(isDisplayed())))
        onView(withId(R.id.tv_feedback)).check(matches(isDisplayed()))
        ...
        onData(allOf(`is`(instanceOf(Currency::class.java)),
            `is`(cachedCurrencies[1]))).perform(click())
    ...
    fun progress_controlsDisabled(){
        ...
        viewModel.data.postValue(Data.loading(null))
